...
// Constants
const ERC1271_ABI = ['function isValidSignature(bytes32 hash, bytes memory signature) view returns (bytes4)'];
...
const hashValid = computed(() => isValidHash(hash.value));
const signatureValid = computed(() => isValidSignature(signature.value));
const rpcValid = computed(
  () => walletConnected.value || (rpcInput.value.trim() !== '' && rpcInput.value.startsWith('http')),
);
const canQuery = computed(
  () => contractAddressValid.value && hashValid.value && signatureValid.value && rpcValid.value && !isEOA.value,
);
...
function isValidSignature(signature: string): boolean {
  return signature.startsWith('0x') && /^0x[0-9a-fA-F]*$/.test(signature);
}
...
async function checkSignatureValidity() {
  ...
  try {
    ...
    const contract = new ethers.Contract(contractAddressInput.value, ERC1271_ABI, provider);
    const result = await contract.isValidSignature(hash.value, signature.value);
    isValid.value = result === '0x1626ba7e';
  }
  catch (error) {
    errorMessage.value = 'Error: Failed to check signature validity';
    console.error(error);
  }
}
...
<template>
  <c-card title="ERC1271 config">
    ...
    <div class="button-container" mb-3>
      <c-button :disabled="!canQuery" @click="checkSignatureValidity">
        Check Signature
      </c-button>
    </div>
    ...
    <input-copyable
      ...
      label-width="100px"
      :value="isValid"
      placeholder="Signature validity status..."
      readonly
      mt-2
    />
